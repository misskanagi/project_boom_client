...
function gooi.newText(params)
    ...
    local inputtype = params.inputtype or "cleartext" --明文："cleartext"，密文："ciphertext"
    x, y, w, h = gooi.checkBounds(
        params.text or defaultText,
        params.x or 10,
        params.y or 10,
    ...
    function f:drawSpecifics(fg)
        ...
        for i = 1, #self.letters do
            ...
            --这一段改造成针对ciphertext和cleartext分别处理
            if self.inputtype == "ciphertext" then
              local radius = ((gooi.getFont(self)):getWidth("w") + 1)/2
              xChar = self.x + mC + charDisplacement + self.mt - self.dispHiddenChar
              yChar = self.y + self.h / 2 - gooi.getFont(self):getHeight() / 2
              charDisplacement = charDisplacement + radius * 2 --每个char占据的宽带是一个w的宽度
              love.graphics.circle("fill", xChar + radius, self.y + self.h / 2, radius - 1)
            else
              love.graphics.print(
                  letter.char,
                  math.floor(xChar),
