--[[ 一个序列化工具，就是可以将table对象序列化:
local serialize = require "./libs/serialize"
table_obj = {.........}
seri_string = serialize.doit(table_obj) -- now the seri_string is what we want
seri_string现在可以通过loadstring函数加载（string本身是一个script），或者保存到文件里以后，使用require或者dofile来运行。
]]--
local serialize = {}
local tinsert, tconcat, infinity = table.insert, table.concat, math.huge

function serialize.doit(value)
