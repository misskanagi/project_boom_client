...
-- 在原生基础上做了以下修改：
--[[
  1. 修改了text控件。使其autosize=false时绘制文字居中。并且有了底色。
]]--
...
Gspot.text = {
	load = function(this, Gspot, label, pos, parent, textmiddle, bgcolor)
		local element = Gspot:element('text', label, pos, parent)
		element:setfont(element.style.font)
    element.bgcolor = bgcolor
    element.textmiddle = textmiddle
		return Gspot:add(element)
	end,  
    ...
    this:drawshape(pos)
		love.graphics.setColor(this.style.labelfg or this.style.fg)
		if this.textmiddle then 
      --lgprint(this.label, pos.x + (this.style.unit / 4), pos.y + ((this.style.unit - this.style.font:getHeight()) / 2))
      lgprint(this.label, pos.x + (this.style.unit / 4), (this.img and pos.y + ((this.style.unit - this.style.font:getHeight()) / 2)) or (pos.y + (pos.h / 2)) - (this.style.font:getHeight() / 2))
    else 
    --lgprintf(this.label, pos.x + (this.style.unit / 4), pos.y + ((this.style.unit - this.style.font:getHeight()) / 2), (this.autosize and pos.w) or pos.w - (this.style.unit / 2), 'left') end
    lgprint(this.label, (pos.x + (pos.w / 2)) - (this.style.font:getWidth(this.label) / 2), (this.img and pos.y + ((this.style.unit - this.style.font:getHeight()) / 2)) or (pos.y + (pos.h / 2)) - (this.style.font:getHeight() / 2))
    end
  end
}
...
Gspot.scrollgroup = {
	load = function(this, Gspot, label, pos, parent, axis, bgcolor)
		axis = axis or 'both'
