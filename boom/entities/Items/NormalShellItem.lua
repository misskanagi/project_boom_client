...
-- shell entity
local createNormalShellItem = function(x, y, r, world, light_world)
    local e = Entity()
    local image = assets.items.item_normal_shell
    ...
    e:add(CollisionCallbacks(
        function(that_entity, coll)
            if that_entity:has("IsPlayer") and that_entity:has("Launchable") then
                ...
                L.shell_name = "NormalShell"
                L.shell_count = 10
                L.launch_sound = assets.sound.shell_normal_launch
